...
always @(posedge clk) begin
    // middle: 320
    if (tick_19bits && pikachu_opp_hpos < ball_hpos && pikachu_opp_hpos < 580) pikachu_opp_hpos <= pikachu_opp_hpos + 1;
...
assign touch_net = ball_hpos >= 250 && ball_hpos <= 340 && ball_vcenter >= 245;
assign net_is_left = ball_hcenter < 295;
assign net_is_below = ball_vcenter >= 245 && ball_vcenter <= 280;
...
always @(posedge clk) begin // speed dir
    if (usr_sw[3]) begin
        ...
        // vdir
        if (touch_top_screen) ball_vdir <= 0;
        else if (touch_ground) ball_vdir <= 1;
...
reg pre_ball_vdir;
always @(posedge clk) pre_ball_vdir <= ball_vdir;
wire ball_vdir_change;
assign ball_vdir_change = ball_vdir != pre_ball_vdir;

always @(posedge clk) begin // speed val
    // vspeed
    if (usr_sw[3]) ball_vspeed <= 5;
    ...
    // hspeed
    if (usr_sw[3]) ball_hspeed <= 0;
    else if (v_touch_usr && h_touch_usr && btn_level_1) ball_hspeed <= 13;
    else if (v_touch_usr && h_touch_usr) ball_hspeed <= 5;
    else if (v_touch_opp && h_touch_opp) ball_hspeed <= 5;
end


always @(posedge clk) begin // pos += speed
    
    if (~usr_sw[3] && tick_21bits) begin
        if (~ball_vdir) begin
